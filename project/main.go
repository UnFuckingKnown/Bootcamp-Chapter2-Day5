...
func update(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(Projectname,StartDate,EndDate,Description,Technologis)

	_, err = connection.Conn.Exec(context.Background(), "UPDATE tb_projects SET  name=$1, start_date=$2, end_date=$3, description=$4, technologies=$5 WHERE id=$6;", Projectname, StartDate, EndDate, Description, Technologis, id)
	
	// _, err = connection.Conn.Exec(context.Background(),"UPDATE public.tb_projects SET id=?, name=?, start_date=?, end_date=?, description=?, technologies=?, image=? WHERE <condition>;")

	// _ , err = connection.Conn.Exec(context.Background(), "UPDATE tb_projects SET name='intizam', start_date='2006-01-02', end_date='2006-01-02', description='intizam', technologies='{\"on\",\"on\"}' WHERE id=$1;",id)
